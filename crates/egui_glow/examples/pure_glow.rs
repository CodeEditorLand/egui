...
impl GlutinWindowContext {
	...
	unsafe fn new(event_loop:&winit::event_loop::EventLoopWindowTarget<()>) -> Self {
		...
		// by default, glutin will try to create a core opengl context. but, if it is
		// not available, try to create a gl-es context using this fallback attributes
		...
		let not_current_gl_context = unsafe {
			gl_display.create_context(&gl_config, &context_attributes).unwrap_or_else(|_| {
				log::debug!(
					"failed to create gl_context with attributes: {:?}. retrying with fallback \
					 context attributes: {:?}",
					&context_attributes,
		...
		// this is where the window is created, if it has not been created while
		// searching for suitable gl_config
		...
		GlutinWindowContext { window, gl_context, gl_display, gl_surface }
	}

...
use egui_wgpu::{RenderState, SurfaceErrorAction, renderer::ScreenDescriptor};
...
impl WebPainterWgpu {
	...
	pub fn render_state(&self) -> Option<RenderState> { self.render_state.clone() }
...
impl WebPainter for WebPainterWgpu {
	...
	fn paint_and_update_textures(
		...
		let render_state = if let Some(render_state) = &self.render_state {
			...
			return Err(JsValue::from_str("Can't paint, wgpu renderer was already disposed"));
		};
		...
		let frame = if is_zero_sized_surface {
				...
				|| size_in_pixels[1] != self.surface_configuration.height
			{
				...
				self.surface.configure(&render_state.device, &self.surface_configuration);
	...
	fn destroy(&mut self) { self.render_state = None; }
}

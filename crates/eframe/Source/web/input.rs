use super::{AppRunner, canvas_element, canvas_origin};
...
/// A single touch is translated to a pointer movement. When a second touch is
/// added, the pointer should not jump to a different position. Therefore, we do
/// not calculate the average position of all touches, but we keep using the
/// same touch as long as it is available.
///
/// `touch_id_for_pos` is the [`TouchId`](egui::TouchId) of the
/// [`Touch`](web_sys::Touch) we previously used to determine the
/// pointer position.
pub fn pos_from_touch_event(
	...
	// Use the touch found above or pick the first, or return a default position if
	// there is no touch at all. (The latter is not expected as the current method
	// is only called when there is at least one touch.)
	touch_for_pos

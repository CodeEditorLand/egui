...
mod glow_integration {
	...
	impl GlowWinitApp {
		...
		fn init_run_state(&mut self, event_loop: &EventLoopWindowTarget<UserEvent>) -> Result<()> {
			...
			self.running = Some(GlowWinitRunning { gl_window, gl, painter, integration, app });
	...
	impl WinitApp for GlowWinitApp {
		...
		fn save_and_destroy(&mut self) {
			if let Some(mut running) = self.running.take() {
				running.integration.save(running.app.as_mut(), running.gl_window.window());
		...
		fn run_ui_and_paint(&mut self) -> EventResult {
			if let Some(running) = &mut self.running {
				...
				let GlowWinitRunning { gl_window, gl, app, integration, painter } = running;
		...
		fn on_event(
			...
			Ok(match event {
				...
				winit::event::Event::UserEvent(UserEvent::AccessKitActionRequest(
					...
					if let Some(running) = &mut self.running {
						running.integration.on_accesskit_action_request(request.clone());
...
mod wgpu_integration {
	...
	impl WgpuWinitApp {
		...
		fn init_run_state(
			...
			self.running = Some(WgpuWinitRunning { painter, integration, app });
			self.window = Some(window);
	...
	impl WinitApp for WgpuWinitApp {
		...
		fn run_ui_and_paint(&mut self) -> EventResult {
			if let (Some(running), Some(window)) = (&mut self.running, &self.window) {
				...
				let WgpuWinitRunning { app, integration, painter } = running;

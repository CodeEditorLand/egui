//! Platform-agnostic interface for writing apps using [`egui`] (epi = egui
...
///
/// You can configure any platform specific details required on top of the
/// default configuration done by `EFrame`.
// #[cfg(not(target_arch = "wasm32"))]
// #[cfg(any(feature = "glow", feature = "wgpu"))]
...
///
/// You can use the [`CreationContext`] to setup egui, restore state, setup
/// OpenGL things, etc.
pub type AppCreator = Box<dyn FnOnce(&CreationContext<'_>) -> Box<dyn App>>;

/// Data that is passed to [`AppCreator`] that can be used to setup and
/// initialize your app.
pub struct CreationContext<'s> {
	/// The egui Context.
	///
	/// You can use this to customize the look of egui, e.g to call
	/// [`egui::Context::set_fonts`], [`egui::Context::set_visuals`] etc.
	pub egui_ctx:egui::Context,

	/// Information about the surrounding environment.
	pub integration_info:IntegrationInfo,

	/// You can use the storage to restore app state(requires the "persistence"
	/// feature).
	pub storage:Option<&'s dyn Storage>,

	/// The [`glow::Context`] allows you to initialize OpenGL resources (e.g.
	/// shaders) that you might want to use later from a
	/// [`egui::PaintCallback`].
	///
	/// Only available when compiling with the `glow` feature and using
	/// [`Renderer::Glow`].
	#[cfg(feature = "glow")]
	...
	///
	/// Only available when compiling with the `wgpu` feature and using
	/// [`Renderer::Wgpu`].
	///
...
pub trait App {
	...
	///
	/// To force a repaint, call [`egui::Context::request_repaint`] at any time
	...
	///
	/// You need to implement this if you want to be able to access the
	/// application from JS using [`crate::web::backend::AppRunner`].
	///
	/// This is needed because downcasting `Box<dyn App>` -> `Box<dyn Any>` to
	/// get &`ConcreteApp` is not simple in current rust.
	///
	...
	/// Called on shutdown, and perhaps at regular intervals. Allows you to save
	/// state.
	///
	...
	/// Called when the user attempts to close the desktop window and/or quit
	/// the application.
	///
	/// By returning `false` the closing will be aborted. To continue the
	/// closing return `true`.
	///
	/// A scenario where this method will be run is after pressing the close
	/// button on a native window, which allows you to ask the user whether
	/// they want to do something before exiting. See the example at <https://github.com/emilk/egui/blob/master/examples/confirm_exit/> for practical usage.
	///
	/// It will _not_ be called on the web or when the window is forcefully
	/// closed.
	#[cfg(not(target_arch = "wasm32"))]
	#[doc(alias = "exit")]
	#[doc(alias = "quit")]
	...
	///
	/// To get a [`glow`] context you need to compile with the `glow` feature
	/// flag, and run eframe with the glow backend.
	#[cfg(feature = "glow")]
	...
	fn auto_save_interval(&self) -> std::time::Duration { std::time::Duration::from_secs(30) }

	/// The size limit of the web app canvas.
	///
	/// By default the max size is [`egui::Vec2::INFINITY`], i.e. unlimited.
	fn max_size_points(&self) -> egui::Vec2 { egui::Vec2::INFINITY }
	...
	///
	/// ATTENTION:
	/// Since these float values go to the render as-is, any color space
	/// conversion as done e.g. by converting from [`egui::Color32`] to
	/// [`egui::Rgba`] may cause incorrect results. egui recommends that
	/// rendering backends use a normal "gamma-space" (non-sRGB-aware) blending,
	///  which means the values you return here should also be in `sRGB`
	/// gamma-space in the 0-1 range. You can use
	/// [`egui::Color32::to_normalized_gamma_f32`] for this.
	...
	///
	/// This can help pre-caching resources loaded by different parts of the UI,
	/// preventing stutter later on.
	///
	...
	fn warm_up_enabled(&self) -> bool { false }
...
pub struct NativeOptions {
	/// Sets whether or not the window will always be on top of other windows at
	/// initialization.
	...
	///
	/// See [winit's documentation][with_fullsize_content_view] for information
	/// on Mac-specific options.
	///
	...
	///
	/// The default icon is a white `e` on a black background (for "egui" or
	/// "eframe"). If you prefer the OS default, set this to `None`.
	pub icon_data:Option<IconData>,

	/// The initial (inner) position of the native window in points (logical
	/// pixels).
	...
	/// You should avoid having a [`egui::CentralPanel`], or make sure its frame
	/// is also transparent.
	pub transparent:bool,

	/// On desktop: mouse clicks pass through the window, used for
	/// non-interactable overlays Generally you would use this in conjunction
	/// with always_on_top
	...
	/// but if you are embedding 3D in egui you may want to turn on
	/// multisampling.
	...
	/// Try to detect and follow the system preferred setting for dark vs light
	/// mode.
	///
	/// The theme will automatically change when the dark vs light mode
	...
	///
	/// When `true`,
	/// [`winit::platform::run_return::EventLoopExtRunReturn::run_return`] is
	/// used. When `false`, [`winit::event_loop::EventLoop::run`] is used.
	pub run_and_return:bool,

	/// Hook into the building of an event loop before it is run.
	///
	/// Specify a callback here in case you need to make platform specific
	/// changes to the event loop before it is run.
	///
	/// Note: A [`NativeOptions`] clone will not include any
	/// `event_loop_builder` hook.
	// #[cfg(any(feature = "glow", feature = "wgpu"))]
	// pub event_loop_builder: Option<EventLoopBuilderHook>,
	#[cfg(feature = "glow")]
	/// Needed for cross compiling for VirtualBox VMSVGA driver with OpenGL ES
	/// 2.0 and OpenGL 2.1 which doesn't support SRGB texture. See <https://github.com/emilk/egui/pull/1993>.
	///
	/// For OpenGL ES 2.0: set this to [`egui_glow::ShaderVersion::Es100`] to
	/// solve blank texture problem (by using the "fallback shader").
	...
	/// The application id, used for determining the folder to persist the app
	/// to.
	///
	/// On native the path is picked using [`directories_next::ProjectDirs::data_dir`](https://docs.rs/directories-next/2.0.0/directories_next/struct.ProjectDirs.html#method.data_dir) which is:
	/// * Linux:   `/home/UserName/.local/share/APP_ID`
	...
	/// * Windows: `C:\Users\UserName\AppData\Roaming\APP_ID`
	///
	/// If you don't set [`Self::app_id`], the title argument to
	/// [`crate::run_native`] will be used instead.
	///
	/// ### On Wayland
...
impl Default for NativeOptions {
	fn default() -> Self {
		Self {
			...
			// We set a default "egui" or "eframe" icon, which is usually more distinctive than the
			// default OS icon.
			icon_data:Some(
...
impl Default for Renderer {
	fn default() -> Self {
		...
		compile_error!(
			"eframe: you must enable at least one of the rendering backend features: 'glow' or \
			 'wgpu'"
		);
	}
}
...
impl std::str::FromStr for Renderer {
	...
	fn from_str(name:&str) -> Result<Self, String> {
		match name.to_lowercase().as_str() {
			...
			_ => {
				Err(format!(
					"eframe renderer {name:?} is not available. Make sure that the corresponding \
					 eframe feature is enabled."
				))
			},
		}
	}
...
pub struct Frame {
	...
	/// A place where you can store custom data in a way that persists when you
	/// restart the app.
	...
	/// If [`Frame::request_screenshot`] was called during a frame, this field
	/// will store the screenshot such that it can be retrieved during
	/// [`App::post_rendering`] with [`Frame::screenshot`]
	#[cfg(not(target_arch = "wasm32"))]
...
impl Frame {
	...
	pub fn is_web(&self) -> bool { cfg!(target_arch = "wasm32") }

	/// Information about the integration.
	pub fn info(&self) -> IntegrationInfo { self.info.clone() }

	/// A place where you can store custom data in a way that persists when you
	/// restart the app.
	pub fn storage(&self) -> Option<&dyn Storage> { self.storage.as_deref() }

	/// Request the current frame's pixel data. Needs to be retrieved by calling
	/// [`Frame::screenshot`] during [`App::post_rendering`].
	#[cfg(not(target_arch = "wasm32"))]
	pub fn request_screenshot(&mut self) { self.output.screenshot_requested = true; }

	/// Cancel a request made with [`Frame::request_screenshot`].
	#[cfg(not(target_arch = "wasm32"))]
	pub fn cancel_screenshot_request(&mut self) { self.output.screenshot_requested = false; }

	/// During [`App::post_rendering`], use this to retrieve the pixel data that
	/// was requested during [`App::update`] via [`Frame::request_screenshot`].
	///
	/// Returns None if:
	/// * Called in [`App::update`]
	/// * [`Frame::request_screenshot`] wasn't called on this frame during
	///   [`App::update`]
	/// * The rendering backend doesn't support this feature (yet). Currently
	///   implemented for wgpu and glow, but not with wasm as target.
	...
	/// ```
	#[cfg(not(target_arch = "wasm32"))]
	pub fn screenshot(&self) -> Option<egui::ColorImage> { self.screenshot.take() }
	...
	///
	/// Note that all egui painting is deferred to after the call to
	/// [`App::update`] ([`egui`] only collects [`egui::Shape`]s and then
	/// eframe paints them all in one go later on).
	///
	/// To get a [`glow`] context you need to compile with the `glow` feature
	/// flag, and run eframe using [`Renderer::Glow`].
	#[cfg(feature = "glow")]
	pub fn gl(&self) -> Option<&std::sync::Arc<glow::Context>> { self.gl.as_ref() }
	...
	#[cfg(not(target_arch = "wasm32"))]
	pub fn set_minimized(&mut self, minimized:bool) { self.output.minimized = Some(minimized); }

	/// Bring the window into focus (native only). Has no effect on Wayland, or
	/// if the window is minimized or invisible.
	///
	/// This method puts the window on top of other applications and takes input
	/// focus away from them, which, if unexpected, will disturb the user.
	#[cfg(not(target_arch = "wasm32"))]
	pub fn focus(&mut self) { self.output.focus = Some(true); }

	/// If the window is unfocused, attract the user's attention (native only).
	///
	/// Typically, this means that the window will flash on the taskbar, or
	/// bounce, until it is interacted with.
	///
	/// When the window comes into focus, or if `None` is passed, the attention
	/// request will be automatically reset.
	///
	/// See [winit's documentation][user_attention_details] for
	/// platform-specific effect details.
	///
	...
	#[cfg(not(target_arch = "wasm32"))]
	pub fn set_maximized(&mut self, maximized:bool) { self.output.maximized = Some(maximized); }

	/// Tell `eframe` to close the desktop window.
	#[cfg(not(target_arch = "wasm32"))]
	#[deprecated = "Renamed `close`"]
	pub fn quit(&mut self) { self.close(); }
	...
	/// If false it will be difficult to move and resize the app.
	#[cfg(not(target_arch = "wasm32"))]
	pub fn set_decorations(&mut self, decorated:bool) { self.output.decorated = Some(decorated); }
	...
	#[cfg(not(target_arch = "wasm32"))]
	pub fn drag_window(&mut self) { self.output.drag_window = true; }

	/// Set the visibility of the window.
	#[cfg(not(target_arch = "wasm32"))]
	pub fn set_visible(&mut self, visible:bool) { self.output.visible = Some(visible); }
...
pub struct WindowInfo {
	/// Coordinates of the window's outer top left corner, relative to the top
	/// left corner of the first display.
	///
...
pub(crate) mod backend {
	...
	pub struct AppOutput {
		/// Set to `true` to close the native window (which often quits the
		/// app).
		#[cfg(not(target_arch = "wasm32"))]
		pub close:bool,

		/// Set to some size to resize the outer window (e.g. glium window) to
		/// this size.
		#[cfg(not(target_arch = "wasm32"))]
		pub window_size:Option<egui::Vec2>,

		/// Set to some string to rename the outer window (e.g. glium window) to
		/// this title.
		#[cfg(not(target_arch = "wasm32"))]
		...
		/// Set to some position to move the outer window (e.g. glium window) to
		/// this position
		#[cfg(not(target_arch = "wasm32"))]
